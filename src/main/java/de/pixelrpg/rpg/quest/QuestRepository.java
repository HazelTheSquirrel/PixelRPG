...
public final class QuestRepository {
    ...
    private void loadJsonDefinitions() {
        ...
        for (var element : definitions) {
            ...
            Quest quest = new Quest(
                    ...
                    string(reward, "companionId", ""),
                    string(json, "questGiverNpcId", ""),
                    string(navigation, "structure", ""),
    ...
    private boolean validateDefinition(JsonObject json) {
        ...
        if (level < Level.MIN_LEVEL || level > Level.MAX_NORMAL_LEVEL || category < Level.MIN_LEVEL || category > Level.MAX_NORMAL_LEVEL || amount <= 0) {
            plugin.getLogger().warning("Ignoring quest '" + id + "': invalid level or amount.");
            return false;
        }

        if (type != QuestType.GLOBAL_EVENT && string(json, "questGiverNpcId", "").isBlank()) {
            plugin.getLogger().warning("Ignoring quest '" + id + "': questGiverNpcId is required.");
            return false;
        }

        switch (type) {
            case HUNT -> {
                if (!isVanillaEntityType(string(json, "targetKey", ""))) {
    ...
    private boolean isVanillaMaterial(String key) {
        ...
        try {
            Material material = Material.valueOf(key.trim().toUpperCase());
    ...
    private boolean isVanillaEntityType(String key) {
        if (key == null || key.isBlank()) return false;
        try {
            org.bukkit.entity.EntityType type = org.bukkit.entity.EntityType.valueOf(key.trim().toUpperCase());
    ...
    private static int number(JsonObject object, String key, int fallback) {
        return object != null && object.has(key) && object.get(key).isJsonPrimitive() && object.getAsJsonPrimitive(key).isNumber()
                ? object.getAsJsonPrimitive(key).getAsInt() : fallback;
    }

    private static long numberLong(JsonObject object, String key, long fallback) {
        return object != null && object.has(key) && object.get(key).isJsonPrimitive() && object.getAsJsonPrimitive(key).isNumber()
                ? object.getAsJsonPrimitive(key).getAsLong() : fallback;
    }

    private static double numberDouble(JsonObject object, String key, double fallback) {
        return object != null && object.has(key) && object.get(key).isJsonPrimitive() && object.getAsJsonPrimitive(key).isNumber()
                ? object.getAsJsonPrimitive(key).getAsDouble() : fallback;
    }

    private static boolean bool(JsonObject object, String key, boolean fallback) {
        return object != null && object.has(key) && object.get(key).isJsonPrimitive() && object.getAsJsonPrimitive(key).isBoolean()
                ? object.getAsJsonPrimitive(key).getAsBoolean() : fallback;
    }
    ...
    private QuestType parseType(String value) {
        try { return QuestType.valueOf(value.trim().toUpperCase()); }
        catch (IllegalArgumentException exception) { return QuestType.HUNT; }
    }
}
