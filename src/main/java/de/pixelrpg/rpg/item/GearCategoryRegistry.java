...
public final class GearCategoryRegistry {
    ...
    public static Optional<ItemCategory> resolve(Material material) {
        ...
        if (name.equals("FIRE_CHARGE")) return Optional.of(ItemCategory.RANGED_WEAPON);
        if (name.contains("BOW") || name.contains("CROSSBOW") || name.contains("TRIDENT")) return Optional.of(ItemCategory.RANGED_WEAPON);
