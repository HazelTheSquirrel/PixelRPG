...
public final class WakeScheduler<K> {
    ...
    public void wake(K key, Runnable action) {
        ...
        scheduled.computeIfAbsent(key, ignored -> plugin.getServer().getScheduler().runTask(plugin, () -> {
    ...
    public void wakeLater(K key, long delayTicks, Runnable action) {
        if (key == null || action == null) return;
        scheduled.computeIfAbsent(key, ignored -> plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            scheduled.remove(key);
            action.run();
