...
public final class CraftingService {
    ...
    public CraftResult craft(Player player, String recipeId) {
        ...
        for (Map.Entry<Material, Integer> cost : recipe.costs().entrySet()) if (!player.getInventory().contains(cost.getKey(), cost.getValue())) return CraftResult.failure("Missing materials");
    ...
    private void makeCraftResultStackable(ItemStack result) {
        ...
        meta.getPersistentDataContainer().remove(de.pixelrpg.rpg.core.RPGKeys.Item.instanceId());
        result.setItemMeta(meta);
    }
    private long craftExperience(CraftRecipe recipe) { return Math.max(20L, recipe.requiredProfessionLevel() * 6L + recipe.costs().values().stream().mapToLong(Integer::longValue).sum() * 5L); }
