...
/** Immutable simple polygon on the X/Z plane. */
public final class RegionGeometry {
    ...
    public static ValidationResult validate(List<RegionPoint> rawPoints) {
        ...
        var points = new ArrayList<>(rawPoints);
        ...
        for (int i = 0; i < points.size(); i++) {
            RegionPoint a = points.get(i);
            RegionPoint b = points.get((i + 1) % points.size());
            if (same(a, b)) return ValidationResult.invalid("Direkt aufeinanderfolgende Punkte dürfen nicht identisch sein.");
        }
        double area = Math.abs(signedArea(points));
        if (area <= EPSILON) return ValidationResult.invalid("Das Polygon hat keine gültige Fläche.");

        int n = points.size();
        for (int i = 0; i < n; i++) {
            RegionPoint a1 = points.get(i);
            RegionPoint a2 = points.get((i + 1) % n);
            for (int j = i + 1; j < n; j++) {
                if (j == i || (j == (i + 1) % n) || (i == 0 && j == n - 1)) continue;
                RegionPoint b1 = points.get(j);
                RegionPoint b2 = points.get((j + 1) % n);
                if (segmentsIntersect(a1, a2, b1, b2)) {
                    return ValidationResult.invalid("Die Polygonkontur überschneidet oder berührt sich selbst zwischen den Kanten " + (i + 1) + " und " + (j + 1) + ".");
                }
            }
        }
