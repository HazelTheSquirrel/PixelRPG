...
public final class MySQLPlayerProfileRepository implements PlayerProfileRepository {
    ...
    @Override public Optional<PlayerProfile> load(UUID uuid) throws SQLException { try (Connection connection = databaseManager.getDataSource().getConnection()) { PlayerProfile profile = loadPlayerRow(connection, uuid); if (profile == null) return Optional.empty(); loadActiveQuests(connection, uuid, profile); loadNavigationTargets(connection, uuid, profile); loadStatistics(connection, uuid, profile); loadEquipment(connection, uuid, profile); profile.markClean(); return Optional.of(profile); } }
    ...
    private void loadNavigationTargets(Connection connection, UUID uuid, PlayerProfile profile) throws SQLException {
        String sql = "SELECT quest_id, world_uuid, x, y, z, target_key FROM pixelrpg_quest_navigation_targets WHERE uuid = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setString(1, uuid.toString());
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    try {
                        profile.setQuestNavigationTarget(
                                resultSet.getString("quest_id"),
                                new PlayerProfile.NavigationTarget(
    ...
    private Profession parseProfessionKey(String raw) { if (raw == null || raw.isBlank()) return null; String normalized = raw.trim().toUpperCase(); if (normalized.equals("PROVISIONER")) return Profession.COOK; if (normalized.equals("MOUNTAIN_MINER")) return Profession.BLACKSMITH; try { return Profession.valueOf(normalized); } catch (IllegalArgumentException ignored) { return null; } }
    ...
    @Override public long save(PlayerProfile profile) throws SQLException {
        ...
        String lockRevisionSql = "SELECT persistence_revision FROM pixelrpg_players WHERE uuid = ? FOR UPDATE"; String deleteQuestsSql = "DELETE FROM pixelrpg_active_quests WHERE uuid = ?"; String insertQuestSql = "INSERT INTO pixelrpg_active_quests (uuid, quest_id, amount, expiry) VALUES (?, ?, ?, ?)"; String deleteNavigationSql = "DELETE FROM pixelrpg_quest_navigation_targets WHERE uuid = ?";
        String insertNavigationSql = "INSERT INTO pixelrpg_quest_navigation_targets (uuid, quest_id, world_uuid, x, y, z, target_key) VALUES (?, ?, ?, ?, ?, ?, ?)";
        String deleteEquipmentSql = "DELETE FROM pixelrpg_player_equipment WHERE uuid = ?"; String insertEquipmentSql = "INSERT INTO pixelrpg_player_equipment (uuid, slot, item_yaml) VALUES (?, ?, ?)"; String deleteStatsSql = "DELETE FROM pixelrpg_player_stats WHERE uuid = ?"; String upsertStatSql = "INSERT INTO pixelrpg_player_stats (uuid, stat_key, value) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE value = VALUES(value)";
        ...
        Map<String, PlayerProfile.NavigationTarget> navigationTargets = profile.getQuestNavigationTargets();
        ...
        try (Connection connection = databaseManager.getDataSource().getConnection()) { connection.setAutoCommit(false); try { Long databaseRevision = null; try (PreparedStatement statement = connection.prepareStatement(lockRevisionSql)) { statement.setString(1, profile.getUuid().toString()); try (ResultSet resultSet = statement.executeQuery()) { if (resultSet.next()) databaseRevision = resultSet.getLong(1); } } if (databaseRevision != null && databaseRevision.longValue() != expectedRevision) throw new SQLException("Stale player profile revision for " + profile.getUuid() + ": memory=" + expectedRevision + ", database=" + databaseRevision); if (databaseRevision == null && expectedRevision != 0L) throw new SQLException("Player profile revision " + expectedRevision + " exists in memory but the database row is missing for " + profile.getUuid());
            ...
            try (PreparedStatement statement = connection.prepareStatement(deleteNavigationSql)) {
                statement.setString(1, profile.getUuid().toString());
                statement.executeUpdate();
            }
            if (!navigationTargets.isEmpty()) try (PreparedStatement statement = connection.prepareStatement(insertNavigationSql)) {
                for (var entry : navigationTargets.entrySet()) {
                    PlayerProfile.NavigationTarget target = entry.getValue();
                    statement.setString(1, profile.getUuid().toString());
                    statement.setString(2, entry.getKey());
                    statement.setString(3, target.worldId().toString());
                    statement.setDouble(4, target.x());
                    statement.setDouble(5, target.y());
                    statement.setDouble(6, target.z());
                    statement.setString(7, target.structureKey());
                    statement.addBatch();
                }
