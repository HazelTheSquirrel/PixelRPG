...
public final class MySQLPlayerProfileRepository implements PlayerProfileRepository {
    ...
    @Override public Optional<PlayerProfile> load(UUID uuid) throws SQLException {
        try (Connection connection = databaseManager.getDataSource().getConnection()) {
            PlayerProfile profile = loadPlayerRow(connection, uuid); if (profile == null) return Optional.empty();
    ...
    @Override public long save(PlayerProfile profile) throws SQLException {
        ...
        String deleteQuestsSql = "DELETE FROM pixelrpg_active_quests WHERE uuid = ?"; String insertQuestSql = "INSERT INTO pixelrpg_active_quests (uuid, quest_id, amount, expiry) VALUES (?, ?, ?, ?)";
        String deleteEquipmentSql = "DELETE FROM pixelrpg_player_equipment WHERE uuid = ?"; String insertEquipmentSql = "INSERT INTO pixelrpg_player_equipment (uuid, slot, item_yaml) VALUES (?, ?, ?)";
        String deleteStatsSql = "DELETE FROM pixelrpg_player_stats WHERE uuid = ?";
        ...
        try (Connection connection = databaseManager.getDataSource().getConnection()) {
            ...
            try {
                Long databaseRevision = null; try (PreparedStatement statement = connection.prepareStatement(lockRevisionSql)) { statement.setString(1, profile.getUuid().toString()); try (ResultSet resultSet = statement.executeQuery()) { if (resultSet.next()) databaseRevision = resultSet.getLong(1); } }
                if (databaseRevision != null && databaseRevision.longValue() != expectedRevision) throw new SQLException("Stale player profile revision for " + profile.getUuid() + ": memory=" + expectedRevision + ", database=" + databaseRevision);
                if (databaseRevision == null && expectedRevision != 0L) throw new SQLException("Player profile revision " + expectedRevision + " exists in memory but the database row is missing for " + profile.getUuid());
                try (PreparedStatement statement = connection.prepareStatement(upsertPlayerSql)) { statement.setString(1, profile.getUuid().toString()); statement.setBoolean(2, profile.isRegistered()); statement.setLong(3, profile.getExperience()); statement.setLong(4, profile.getMoneyMinorUnits()); statement.setString(5, String.join(",", profile.getUnlockedWaypoints())); statement.setInt(6, profile.getStoryChapterIndex()); statement.setString(7, String.join(",", profile.getCompletedQuests())); statement.setBoolean(8, profile.isScoreboardEnabled()); statement.setBoolean(9, profile.isPartyHudEnabled()); statement.setBoolean(10, profile.isQuestTrackerEnabled()); statement.setLong(11, profile.getPlaytimeMillis()); statement.setLong(12, nextRevision); statement.executeUpdate(); }
                ...
                try (PreparedStatement statement = connection.prepareStatement(deleteStatsSql)) { statement.setString(1, profile.getUuid().toString()); statement.executeUpdate(); }
